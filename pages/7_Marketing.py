...
if st.session_state.google_ads_df is None and st.session_state.meta_ads_df is None:
    ...
    if len(combined_df) > 0:
        ...
        # ===== VISUALIZATION A: Customer Journey Volume Funnel =====
        st.markdown("---")
        st.markdown("### Customer Journey Funnel")

        # Build funnel data
        total_reach = see_metrics['reach'] + think_metrics['reach'] + do_metrics['reach']
        total_clicks = see_metrics['clicks'] + think_metrics['clicks'] + do_metrics['clicks']
        total_conversions = see_metrics['conversions'] + think_metrics['conversions'] + do_metrics['conversions']

        funnel_stages = [
            {'stage': 'Reach', 'volume': total_reach, 'color': '#3498db'},
            {'stage': 'Clicks', 'volume': total_clicks, 'color': '#f39c12'},
            {'stage': 'Conversions', 'volume': total_conversions, 'color': '#27ae60'},
        ]

        # Add rebookings if available
        if rebookings > 0:
            funnel_stages.append({'stage': 'Rebookings', 'volume': rebookings, 'color': '#9b59b6'})

        funnel_df = pd.DataFrame(funnel_stages)

        # Calculate percentage of previous stage
        funnel_df['pct_of_previous'] = 100.0
        for i in range(1, len(funnel_df)):
            if funnel_df.iloc[i-1]['volume'] > 0:
                funnel_df.loc[funnel_df.index[i], 'pct_of_previous'] = (
                    funnel_df.iloc[i]['volume'] / funnel_df.iloc[i-1]['volume'] * 100
                )

        # Normalize bars: percentage of max (Reach) with minimum width of 5%
        max_volume = funnel_df['volume'].max()
        if max_volume > 0:
            funnel_df['normalized'] = funnel_df['volume'] / max_volume * 100
            # Apply minimum width of 5% so small values are visible
            funnel_df['normalized'] = funnel_df['normalized'].apply(lambda x: max(x, 5) if x > 0 else 0)
        else:
            funnel_df['normalized'] = 0

        # Create normalized horizontal bar chart
        fig_funnel = go.Figure()

        # Add normalized bars
        fig_funnel.add_trace(go.Bar(
            y=funnel_df['stage'],
            x=funnel_df['normalized'],
            orientation='h',
            marker_color=funnel_df['color'],
            hovertemplate='%{y}: %{customdata:,.0f}<extra></extra>',
            customdata=funnel_df['volume']
        ))

        # Add annotations: actual values on the right, drop-off percentages between bars
        annotations = []

        # Add actual volume labels to the right of each bar
        for i, row in funnel_df.iterrows():
            annotations.append(dict(
                x=105,  # Position to the right of max bar
                y=row['stage'],
                text=f"<b>{row['volume']:,.0f}</b>",
                showarrow=False,
                xanchor='left',
                font=dict(size=14, color='#333')
            ))

        # Add drop-off percentage annotations between bars
        for i in range(1, len(funnel_df)):
            pct = funnel_df.iloc[i]['pct_of_previous']
            annotations.append(dict(
                x=50,  # Center position
                y=(len(funnel_df) - i - 0.5),  # Between bars
                text=f"↓ {pct:.1f}%",
                showarrow=False,
                xanchor='center',
        ...
        fig_funnel.update_layout(
            ...
            xaxis=dict(
                ...
                range=[0, 130]  # Extra space for labels
            ),
            yaxis=dict(
                title='',
                categoryorder='array',
                categoryarray=list(reversed(funnel_df['stage'].tolist())),
                showgrid=False
            ),
